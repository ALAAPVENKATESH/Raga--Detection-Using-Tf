...
class CRePE:
    ...
    def predict_pitches(self, audio):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                p = self.pitch_model.predict(frames, verbose=0)  # Add verbose=0
...
class SPD_Model:
    ...
    def get_hist_emb(self, hist_cqt, note_dim, indices, topk, drop_rate=0.2):
        ...
        hist_cc_all_bt = Lambda(
            lambda pair: tf.stack([tf.roll(pair[0], -pair[1][i], axis=1) for i in range(topk)], axis=1),
            output_shape=(topk, 60, 4)
        )([hist_cqt_b, indices])
    ...
    def get_tonic_emb(self, hist_cqt, note_dim, drop_rate=0.2):
        ...
        tonic_logits = Dense(60, activation='sigmoid')(hist_emb)  # (batch*topk, 60)
        # Restore (batch, topk, 60)
        tonic_bt = Lambda(lambda x: tf.reshape(x, (-1, topk, 60)), output_shape=(topk, 60))(tonic_logits)
        # Roll along note axis by per-topk shift then mean across topk
        rolled_bt = Lambda(lambda pair: tf.stack([tf.roll(pair[0][:, i, :], pair[1][i], axis=1) for i in range(topk)], axis=1), output_shape=(topk, 60))([tonic_bt, indices])
    ...
    def _compute_label_scores(self, pitchvalue_prob):
        ...
        models_weights = np.expand_dims(self.models_weights, 1)  # (25,1)
        raw_scores = np.matmul(pred_proba, models_weights)[:, 0]  # (n_labels,)
        raw_scores = raw_scores - np.max(raw_scores)
        exp_scores = np.exp(raw_scores)
        probs = exp_scores / (np.sum(exp_scores) + 1e-8)
        return probs

    def _segment_and_aggregate_scores(self, pitchvalue_prob, min_frames=800, segment_frames=1500):
        n_frames = pitchvalue_prob.shape[0]
        if n_frames <= min_frames:
            return self._compute_label_scores(pitchvalue_prob)
        scores_accum = None
        start = 0
        segments = 0
        while start < n_frames:
            ...
            if end - start < min_frames:
                break
            seg_scores = self._compute_label_scores(pitchvalue_prob[start:end])
            if scores_accum is None:
                scores_accum = seg_scores
            else:
                scores_accum += seg_scores
            segments += 1
            start = end
        if segments == 0:
            return self._compute_label_scores(pitchvalue_prob)
        scores = scores_accum / segments
        scores = scores / (np.sum(scores) + 1e-8)
        return scores

    def _estimate_note_presence(self, rolled_pitch_prob):
        time_sum = np.sum(rolled_pitch_prob, axis=0)  # (120,)
        note_presence = np.zeros(12)
        for k in range(12):
            note_presence[k] = np.sum(time_sum[k*10:(k+1)*10])
        note_presence = note_presence / (np.sum(note_presence) + 1e-8)
        return note_presence

    def _recheck_pentatonic_derivatives(self, top1_label, top1_conf, note_presence):
        suggestion = None
        ...
        if self.tradition == 'Carnatic' and top1_label == 'sankarabharanam':
            if (top1_conf < 0.6) and m_absent and d_absent and n_present:
                suggestion = ('hamsadhvani', float(min(0.85, 0.5 + 0.5 * (1 - note_presence[M]) * (1 - note_presence[D]) * note_presence[N])))
        if self.tradition == 'Hindustani' and top1_label == 'bilaval':
            if (top1_conf < 0.6) and m_absent and d_absent and n_present:
